...
@interface CNKI_Z_MBProgressHUD () {
	BOOL useAnimation;
...
@implementation CNKI_Z_MBProgressHUD
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	CNKI_Z_MBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	CNKI_Z_MBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
		return YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [CNKI_Z_MBProgressHUD allHUDsForView:view];
	for (CNKI_Z_MBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (CNKI_Z_MBProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[CNKI_Z_MBRoundProgressView class]];
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		[indicator removeFromSuperview];
		self.indicator = MB_AUTORELEASE([[CNKI_Z_MBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[CNKI_Z_MBRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(CNKI_Z_MBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
...
@implementation CNKI_Z_MBRoundProgressView
...
@implementation CNKI_Z_MBBarProgressView
