...
#ifndef UIImagezExtensions
#define UIImagezExtensions
...
CGImageRef CreateGradientImage (int pixelsWide, int pixelsHigh, CGFloat endPoint) {
    ...
    CGContextRef gradientBitmapContext = CGBitmapContextCreate(NULL, pixelsWide, pixelsHigh,8, 0, colorSpace, 0);
...
- (UIImage *)imageToFitSize:(CGSize)fitSize method:(MGImageResizingMethod)resizeMethod
{
	...
	if (!image) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, scaledWidth, scaledHeight, 8, (scaledWidth * 4),colorSpace, 1);
...
{
    UIGraphicsBeginImageContextWithOptions(frame.size, 1, [UIScreen mainScreen].scale); //防止模糊
...
- (UIImage *)imageRotatedByRadians:(CGFloat)radians
{
    ...
    //return [self imageRotatedByDegrees:ExRadiansToDegrees(radians)];
}
- (UIImage *)imageRotatedByDegrees:(CGFloat)degrees
{
    ...
    //CGAffineTransform t = CGAffineTransformMakeRotation(ExDegreesToRadians(degrees));
    float r1=degrees * M_PI / 180;
    CGAffineTransform t = CGAffineTransformMakeRotation(r1);
    ...
    //CGContextRotateCTM(bitmap, ExDegreesToRadians(degrees));
    float r2 = degrees * M_PI / 180;
    CGContextRotateCTM(bitmap, r2);
...
/// 根据颜色生成图片，默认 1x1 point size
+ (UIImage *)z_imageWithColor:(UIColor *)color
{
    return [self z_imageWithColor:color size:CGSizeMake(1, 1)];
}
